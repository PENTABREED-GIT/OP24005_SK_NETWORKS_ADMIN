...
<div id="kt_aside" class="aside aside-dark aside-hoverable" data-kt-drawer="true" data-kt-drawer-name="aside"
    ...
    <div class="aside-logo flex-column-auto" id="kt_aside_logo">
        <!--begin::Logo-->
        <a href="/"><img src="/assets/images/logos/logo-1.png"/></a>
        <!--end::Logo-->
        <!--begin::Aside toggler-->
    ...
    <div class="aside-menu flex-column-fluid">
        ...
        <div class="hover-scroll-overlay-y my-5 my-lg-5" id="kt_aside_menu_wrapper" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-height="auto" data-kt-scroll-dependencies="#kt_aside_logo, #kt_aside_footer" data-kt-scroll-wrappers="#kt_aside_menu" data-kt-scroll-offset="0">
            ...
            <c:forEach items="${LEFT_LIST}" var="leftMenu" varStatus="status">
                <c:choose>
                    <c:when test="${leftMenu.depth == '1' && LEFT_LIST[status.index + 1].depth != '2'}">
                <div class="menu-item">
                    <a class="menu-link <c:if test="${leftMenu.url == PAGE_INFO.url }">active</c:if>" href="<c:out value="${leftMenu.url }"/>">
                    ...
                    <c:when test="${leftMenu.depth == '2' && LEFT_LIST[status.index + 1].depth == '3'}">
                        <div data-kt-menu-trigger="click" class="menu-item menu-accordion <c:if test="${leftMenu.menuIndex == PAGE_INFO.parentIndex }">hover show</c:if>">
                    ...
                    <c:when test="${leftMenu.depth == '3'}">
                        ...
                        <c:if test="${LEFT_LIST[status.index + 1].depth == '2' || status.last}">
                                </div>
                            </div>
                        </c:if>
                    </c:when>
                </c:choose>
                <c:if test="${ (leftMenu.depth == '3' && LEFT_LIST[status.index + 1].depth == '2') || leftMenu.depth == LEFT_LIST[status.index + 1].depth || status.last}">
                        </div>
                </c:if>
            </c:forEach>
            <!--end::Menu-->
        ...
        <!--end::Aside menu-->
        <!--begin::Footer-->
        <div class="aside-footer flex-column-auto pt-5 pb-7 px-5" id="kt_aside_footer"></div>
        <!--end::Footer-->
    </div>
    <!--end::Aside-->
</div>
