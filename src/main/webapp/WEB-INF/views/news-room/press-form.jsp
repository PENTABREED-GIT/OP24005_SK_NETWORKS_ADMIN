                ...
                <div class="post d-flex flex-column-fluid" id="kt_post">
                    ...
                    <div id="kt_content_container" class="container-fluid">
                        ...
                        <form name="detailForm" method="post">
                            ...
                            <input type="hidden" name="pressIndex" value="${detail.pressIndex}"/>
                            <!--begin::Card-->
                            <div class="card mb-7">
                                <!--begin::Card body-->
                                <div class="card-body">
                                    ...
                                    <div class="py-3 row border-bottom border-bottom-dashed border-gray-200 border-bottom border-bottom-dashed border-gray-200">
                                        <label class="col-sm-3 col-md-3 col-xl-2 d-none d-sm-block col-form-label text-end required fw-bolder fw-bolder w-xxl-200px">등록 메뉴</label>
                                        <label class="d-block d-sm-none  required fw-bolder fw-bolder w-xxl-200px">등록 메뉴</label>
                                        <div class="col-sm-9 col-md-9 col-xl-10 col-xxl-9 fv-row">
                                            <div class="form-check form-check-inline col-form-label">
                                                <input class="form-check-input" type="checkbox" name="isPress" value="Y" id="isPress"<c:if test="${detail.isPress eq 'Y'}"> checked</c:if>/>
                                                <label class="form-check-label" for="isPress">프레스 센터</label>
                                            </div>
                                            <div class="form-check form-check-inline col-form-label">
                                                <input class="form-check-input" type="checkbox" name="isBusiness" value="Y" id="isBusiness"<c:if test="${detail.isBusiness eq 'Y'}"> checked</c:if>/>
                                                <label class="form-check-label" for="isBusiness">비즈니스</label>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="py-3 row border-bottom border-bottom-dashed border-gray-200">
                                    ...
                                    <div id="regDateDiv" class="py-3 row border-bottom border-bottom-dashed border-gray-200">
                                        <label class="col-sm-3 col-md-2 d-none d-sm-block col-form-label text-end required fw-bolder w-xxl-200px" for="title">등록일</label>
                                        <label class="d-block d-sm-none  required fw-bolder w-xxl-200px" for="title">등록일</label>
                                        <div class="col-sm-8 col-md-8 col-xl-8 col-xxl-8 fv-row">
                                            <input type="text" class="form-control" id="regDate" name="regDate" placeholder="제목" required maxlength="100" value="<c:out value="${detail.regDate}" />">
                                        </div>
                                    </div>
                                    <div class="py-3 row border-bottom border-bottom-dashed border-gray-200 border-bottom border-bottom-dashed border-gray-200">
                                    ...
                                    <div class="py-3 row border-bottom border-bottom-dashed border-gray-200">
                                        <label class="col-sm-3 col-md-2 d-none d-sm-block col-form-label text-end fw-bolder w-xxl-200px" for="content">내용</label>
                                        <label class="d-block d-sm-none fw-bolder w-xxl-200px" for="content">내용</label>
                                        <div class="col-sm-9 col-md-10 col-xl-10 col-xxl-9 fv-row">
                                            <textarea class="form-control" rows="5" id="content" minlength="1" name="content" placeholder="Content">${detail.content}</textarea>
                                            <script type="text/javascript">
    $(document.detailForm.content).froalaEditor({
        key: ${froalaKey},
            ...
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                                ...
                                <div class="card-footer d-flex justify-content-end">
                                    <div class="col d-flex justify-content-start">
                                        <a class="btn btn-dark" href="javascript: location.href='/news-room/press-list'+location.search;">목록</a>
                                    </div>
...
<script type="text/javascript">
    ...
    window.onload = function() {
        ...
        // regDateDiv 요소를 가져옴
        var regDateDiv = document.getElementById("regDateDiv");

        // URL에 'press-mod'가 포함되어 있는지 확인
        if (currentURL.includes('press-mod')) {
            // 포함되어 있으면 regDateDiv를 보이게 함
            regDateDiv.style.display = 'flex';
        } else {
            // 포함되어 있지 않으면 regDateDiv를 숨김
    ...
    FormValidation.validators.checkLang = function() {
        return {
            validate: function() {
                if(!document.querySelector('input[name=isPress]').checked && !document.querySelector('input[name=isIrNews]').checked && !document.querySelector('input[name=isBusiness]').checked) {
                    return {
                        valid: false
                    };
                }
    ...
    fields['isBusiness'] = {
        selector: 'input[name=isPress],input[name=isIrNews],input[name=isBusiness]',
        validators: {
            checkLang: {
                message: '등록 메뉴를 선택하세요.'
            }
        }
    };
    ...
    async function saveFormData() {
        let detailUrl = '/news-room/press';
        let url = '/api/v1'+detailUrl;
        let method = 'POST';
        let message = '등록되었습니다.';

        // 수정 모드일 때 변수 조정
        if (document.querySelector('input[name=pressIndex]').value !== '') {
            url += '/update/' + uid;
            message = '수정되었습니다.'
        }
