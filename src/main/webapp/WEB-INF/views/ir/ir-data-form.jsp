                ...
                <div class="post d-flex flex-column-fluid" id="kt_post">
                    ...
                    <div id="kt_content_container" class="container-fluid">
                        ...
                        <form name="detailForm" method="post">
                            ...
                            <input type="hidden" name="irDataIndex" value="${detail.irDataIndex}"/>
                            ...
                            <div class="card mb-7">
                                ...
                                <div class="card-body">
                                    <div class="py-3 row border-bottom border-bottom-dashed border-gray-200">
                                        <label class="col-sm-3 col-md-3 col-xl-2 d-none d-sm-block col-form-label text-end required fw-bolder w-xxl-200px">자료 언어 선택</label>
                                        <label class="d-block d-sm-none  required fw-bolder w-xxl-200px">자료 언어 선택</label>
                                        <div class="col-sm-9 col-md-9 col-xl-10 col-xxl-9 fv-row">
                                            <select class="form-select mw-300px" aria-label="자료 언어 선택" name="dataLang" required>
                                                <option value="">자료 언어 선택</option>
                                                <option value="KO"<c:if test="${detail.dataLang eq 'KO'}"> selected</c:if>>국문</option>
                                                <option value="EN"<c:if test="${detail.dataLang eq 'EN'}"> selected</c:if>>영문</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="py-3 row border-bottom border-bottom-dashed border-gray-200">
                                ...
                                <div class="py-3 row">
                                    <label class="col-sm-3 col-md-3 col-xl-2 d-none d-sm-block col-form-label text-end fw-bolder w-xxl-200px required" for="dz0">첨부파일</label>
                                    <label class="d-block d-sm-none  fw-bolder w-xxl-200px required" for="dz0">첨부파일</label>
                                    <div class="col-sm-9 col-md-9 col-xl-10 col-xxl-9 fv-row">
                                        <div class="row form-check col-form-label form-check-custom mw-1000px" id="dz0Container">
                                            ...
                                            <jsp:include page="/file-form-inc" flush="false">
                                                ...
                                                <jsp:param name="parentTable" value="IR_DATA" />
                                ...
                                <div class="card-footer d-flex justify-content-end">
                                    ...
                                    <c:choose>
                                        <c:when test="${classification eq 'RESULT' or detail.classification eq 'RESULT'}">
                                        <a class="btn btn-dark" href="javascript: location.href='/ir/management-report-list'+location.search;">목록</a>
                                        </c:when>
                                        <c:otherwise>
                                        <a class="btn btn-dark" href="javascript: location.href='/ir/ir-data-list'+location.search;">목록</a>
                                        </c:otherwise>
                                    </c:choose>
...
<script type="text/javascript">
    ...
    async function saveFormData() {
        let detailUrl = '/ir/ir-data';
        let url = '/api/v1'+detailUrl;
        let method = 'POST';
        let message = '등록되었습니다.';

        // 수정 모드일 때 변수 조정
        if (document.querySelector('input[name=irDataIndex]').value !== '') {
            url += '/update/' + uid;
            message = '수정되었습니다.'
        }
        ...
        if(response.ok) { // success
            ...
            if(res.result) {
                Swal.fire({
                    ...
                    <c:choose>
                    <c:when test="${classification eq 'RESULT' or detail.classification eq 'RESULT'}">
                    window.location.href = '/ir/management-report/' + uid+location.search;
                    </c:when>
                    <c:otherwise>
                    window.location.href = '/ir/ir-data/' + uid+location.search;
                    </c:otherwise>
                    </c:choose>
                });
            }
