...
public class KISA_SHA256 {
    ...
    public static String Encrpyt(String strValue) {
        String hashStr = "";
        byte pbCipher[]   = new byte[32];
        if( NTUtil.isEmpty(strValue) ) {
            return "";
        }
        byte[] enc = strValue.getBytes();
        SHA256_Encrpyt( enc, strValue.getBytes().length, pbCipher );
        for (int i=0; i<pbCipher.length; i++) {
            hashStr = hashStr + Integer.toHexString(0xff&pbCipher[i]);
        }
        return hashStr;
    }
